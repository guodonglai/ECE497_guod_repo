...
//	alert(i+","+j+" clicked");
//    console.log("lines=%/s", disp.toString);
//	socket.emit('i2c', i2cNum);
    // Toggle bit on display
//if green bit is one, check the red bit
    if(disp[2*i]>>j&0x1 === 1) {
//if red bit is also one, it was orange before.
...
//if red bit is zero, it was green before.
	else{
...
//if red bit is one, it was red
	if(disp[2*i+1]>>j&0x1===1){
          $('#id'+i+'_'+j).removeClass('red');
//turn off the red bit
          disp[2*i+1]^=0x1<<j;
          socket.emit('i2cset', {i2cNum: i2cNum, i: 2*i+1, disp: '0x'+disp[2*i+1].toString(16)});
        }
//if red bit is also zero, it was blank
        else{
...
//	disp2=disp.splice(8);
        ...
        for (i = 0; i < 16; i++) {
            ...
            for (j = 0; j < 8; j++) {
//[green,red]==[1,1] means orange
//[green,red]==[1,0] means green
//[green,red]==[0,1] mean  red
//[green,red]==[0,0] means blank
                if ((((disp[2*i] >> j) & 0x1) === 1)&&(((disp[2*i+1]>>j)&0x1)===1)) {
                     $('#id' + i + '_' + j).addClass('orange');
                }else if ((((disp[2*i] >> j) & 0x1) === 1)&&(((disp[2*i+1]>>j)&0x1)===0)){
                     $('#id' + i + '_' + j).removeClass('orange')
		     $('#id' + i + '_' + j).addClass('on');
		}else if ((((disp[2*i] >> j) & 0x1) === 0)&&(((disp[2*i+1]>>j)&0x1)===1)){
                     $('#id' + i + '_' + j).addClass('on')
		     $('#id' + i + '_' + j).addClass('red');
                }
		else{
